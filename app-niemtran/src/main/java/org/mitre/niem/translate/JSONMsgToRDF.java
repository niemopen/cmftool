...
public class JSONMsgToRDF {
    ...
    private final static String SEQ_URI = "http://niemtran.niem.mitre.org/SeqURI";
    private final static String RDF_TYPE_URI = RDF.type.getURI();
    ...
    public void convert (String msg, Writer w) throws IOException, NIEMTranException {
        ...
        try {
            var me = JsonParser.parseString(msg);
    ...
    public void convert (JsonObject msg, Writer w) throws IOException, NIEMTranException {
        ...
        if (null != context) msg.add("@context", context);
        ...
        while (!deq.isEmpty()) {
            ...
            for (int i = entries.size()-1; i >= 0; i--) {
                ...
                var val = e.getValue();
                var ctQ = "";
                if (SEQ_URI.equals(key)) continue;
                ...
                if (val.isJsonObject()) {
                    ...
                    if (!ctQ.isEmpty()) vobj.addProperty("@type", ctQ);
                    deq.addFirst(vobj);
                }
                    ...
                    for (int j = ja.size()-1; j >=0; j--) {
                        ...
                        if (ae.isJsonObject()) {
                            ...
                            if (!ctQ.isEmpty()) ao.addProperty("@type", ctQ);
                            deq.addFirst(ao);
                        }
                    }
                }
        ...
        var mtxt = new GsonBuilder().disableHtmlEscaping().setPrettyPrinting().create().toJson(msg);
        ...
        for (var stmt : stmtL) {
            ...
            var pred = stmt.getPredicate();
            var obj  = stmt.getObject();
            if (SEQ_URI.equals(pred.getURI())) continue;
            
            var sid = FmtUtils.stringForNode(subj.asNode(), pm);
            if (subj.isAnon()) sid = bnid.getOrDefault(sid, sid);
            
            var pnam = FmtUtils.stringForNode(pred.asNode(), pm);
            if (pred.equals(RDF.type)) pnam = "a";
            
            var objS = FmtUtils.stringForNode(obj.asNode(), pm);
            if (obj.isAnon()) objS = bnid.getOrDefault(objS, objS);
    ...
    public static String expandCompactIRI (JsonObject cxtO, String key) throws NIEMTranException {
        ...
        if (null == base) return null;              // prefix isn't mapped, oh well
    ...
    private String jsonSyntaxExMsg (JsonSyntaxException ex) {
        ...
        if (pos > 0) exm = exm.substring(pos);
        pos = exm.indexOf(" path");
        if (pos > 0) exm = exm.substring(0, pos);
        return exm;
    }
}
