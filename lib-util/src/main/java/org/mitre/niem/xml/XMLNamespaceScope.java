 ...
 * applications that need to resolve QNames that appear in the XML data as the
 * SAX parser trolls merrily along.
 * 
 * @author Scott Renner
...
public class XMLNamespaceScope {
    ...
    /**
     * Convert a QName into Clark notation; eg. {namespaceURI}localName.\
     * Returns null if the argument is not a valid QName or has no binding in
     * the current scope.
     * @param str QName to convert
     * @return equivalent in Clark notation
     */
    public String clarkNotation (String str) {
        var expanded = expandQName(str);
        if (null == expanded) return null;
        if (null == expanded.getValue0()) return "{}"+expanded.getValue1();
        return "{" + expanded.getValue0() + "}" + expanded.getValue1();
    }
